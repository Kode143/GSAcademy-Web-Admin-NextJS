...
export default function Nav(){
    const inactiveLink= 'flex gap-1 p-1 hover:bg-slate-50';
    const activeLink = inactiveLink+' bg-white text-dark rounded-l-lg';
    const router = useRouter();
    const {pathname}= router
    return(
        <aside className="p-2  pr-0 mb-0 ms-2 me-0 ">
            <Link href={'/'} className="flex gap-1 mb-5 mt-4 mr-6 ">
            ...
            <Link href={'/message'} className={pathname.includes('/message') ? activeLink : inactiveLink }>
